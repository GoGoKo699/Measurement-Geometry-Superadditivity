...
The original baseline status text is preserved unchanged in [website/provenance/baseline_STATUS.md](website/provenance/baseline_STATUS.md). Its historical pre-repository wording does not describe the current stage. That original reader pass changed README navigation, this status page, and their manifest entries. Subsequent documentary corrections are recorded below.

## Corrections following the independent repository audit

The audit of commit `f0015c56a19fd953c6797b2c64d9b507105234e3` is saved on `audit/repository-sanity-01` at commit `5e367b1e541bf7d5ba1af90e85796d59db9e03f4`. It independently checked the retained science and completed the full numerical reproduction. The subsequent authorized corrections qualify the historical dephrasure comparison, fix eight figure-input paths, and remove an obsolete pre-repository status clause. [Current attribution note](docs/REFERENCES.md#s6-correction) · [Exact editorial changes](provenance/EDITORIAL_CORRECTIONS.json)

Historical source excerpts and the original audit evidence remain unchanged. Local server examples now bind to `127.0.0.1`. Browser verification remains incomplete because standard Chromium acquisition failed; Pandoc 3.1.11.1 was also unavailable. The successful static and loopback HTTP checks do not establish live browser behavior. Manuscript drafting and public deployment remain later decisions.
