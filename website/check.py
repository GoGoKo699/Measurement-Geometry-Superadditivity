...
def check(site):
 ...
 files=list(site.rglob('*.html'));require(len(files)==len(config['pages']),'Unexpected HTML page count')
 for p in files:
  ...
  for tag,key in [('a','href'),('img','src'),('script','src'),('link','href')]:
   for el in soup.find_all(tag):
    ...
    if u.fragment and dest.suffix=='.html':
     ...
     if not ds.find(id=unquote(u.fragment)):missing.append((p.name,val,'missing anchor'))
    links+=1
 ...
 return {'passed':True,'html_pages':len(files),'local_links_checked':links,'external_links_not_fetched':len(set(outside)),
